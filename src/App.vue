    ...
    <main class="main my-1">
      ...
      <div class="tab-content container">
        <div v-show="activeTab === 'personalInfo'">
          <FormPersonalInfo
            v-bind:personalInfo="currentFormInfo ? currentFormInfo.personal_info: undefined"
            v-on:onNoFixedAddressClick="handleNoFixedAddressClick"
            v-on:onJobSelectChange="handleJobSelectChange"
            v-on:onNoteInput="handleNoteInput"
          ></FormPersonalInfo>
        </div>
        <div v-show="activeTab === 'orders'">
          <FormOrders
            v-bind:orders="currentFormInfo ? currentFormInfo.orders : undefined"
            v-on:onApplePlusMinusClick="handleApplePlusMinusClick"
            v-on:onAppleCountInput="handleAppleCountInput"
          ></FormOrders>
        </div>
        <div v-show="activeTab === 'overview'">
          <FormOverview v-bind:formInfo="currentFormInfo"></FormOverview>
        </div>
      </div>
    </main>
    <Pagination
...
export default {
  ...
  methods: {
    ...
    handleJobSelectChange(e) {
      ...
      this.currentFormInfo.personal_info.job = selectedJob;
    },
    handleNoteInput(e) {
      const { value: note } = e.target;
      this.currentFormInfo.personal_info.note = note;
    },
